...
namespace USAACE.eStaffing.Web
{
    public partial class Site : MasterPage, ISiteView
    {
        ...
        public IList<FormType> FormTypes
        {
            set
            {
                MenuItem listForm = mnuMainMenu.FindItem("ListForm");
                listForm.ChildItems.Clear();

                foreach (FormType formType in value)
                {
                    MenuItem listFormItem = new MenuItem(String.Format("List {0}s", formType.FormTypeName), null, null, NavigateUtil.GetFormListLink(formType));

                    listForm.ChildItems.Add(listFormItem);
                }
            }
        }
